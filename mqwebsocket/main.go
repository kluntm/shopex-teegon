...
import (
	...
	"git.ishopex.cn/teegon/mqservice/deamon"
...
func main() {
	...
	if err := gApp.LoadConfig(*confPath); err != nil {
		fmt.Errorf("load config failed, error:", err.Error())
		return
	}

	if gApp.config.Daemon {
		deamon.Daemon()
		deamon.Partner("./etc/mqwebsocket.pid", os.Args)
	}
